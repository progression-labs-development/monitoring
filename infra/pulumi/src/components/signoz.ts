  ...
  const signozTaskDefinition = new aws.ecs.TaskDefinition(`${name}-task`, {
    ...
    containerDefinitions: pulumi.interpolate`[
      ...
      },
      {
        ...
        "essential": true,
        "entryPoint": ["sh", "-c"],
        "command": ["echo 'receivers: {otlp: {protocols: {grpc: {endpoint: 0.0.0.0:4317}, http: {endpoint: 0.0.0.0:4318}}}}' > /tmp/config.yaml && echo 'processors: {batch: {send_batch_size: 10000, timeout: 10s}}' >> /tmp/config.yaml && echo 'extensions: {health_check: {endpoint: 0.0.0.0:13133}}' >> /tmp/config.yaml && echo 'exporters: {clickhousetraces: {datasource: tcp://clickhouse.signoz.local:9000/signoz_traces, use_new_schema: true}, signozclickhousemetrics: {dsn: tcp://clickhouse.signoz.local:9000/signoz_metrics}, clickhouselogsexporter: {dsn: tcp://clickhouse.signoz.local:9000/signoz_logs, timeout: 10s, use_new_schema: true}}' >> /tmp/config.yaml && echo 'service: {extensions: [health_check], pipelines: {traces: {receivers: [otlp], processors: [batch], exporters: [clickhousetraces]}, metrics: {receivers: [otlp], processors: [batch], exporters: [signozclickhousemetrics]}, logs: {receivers: [otlp], processors: [batch], exporters: [clickhouselogsexporter]}}}' >> /tmp/config.yaml && /signoz-otel-collector --config=/tmp/config.yaml"],
        "portMappings": [
