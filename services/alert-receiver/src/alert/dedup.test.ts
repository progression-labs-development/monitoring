...
import type { IncidentPayload, IncidentResponse } from "../incident/client";
...
describe("dedup", () => {
  it("filters out payloads with matching open incidents", () => {
    const payloads = [makePayload("signoz:fp-1"), makePayload("signoz:fp-2")];
    const open = [makeIncident("signoz:fp-1")];
    const result = dedup(payloads, open);
    expect(result).toHaveLength(1);
    expect(result[0].fingerprint).toBe("signoz:fp-2");
  });
  ...
  it("returns empty when all are duplicates", () => {
    const payloads = [makePayload("signoz:fp-1")];
    const open = [makeIncident("signoz:fp-1")];
    const result = dedup(payloads, open);
